...
public class AutoTestClient {
    public static void main(String[] args) throws IOException,ClassNotFoundException,NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        String filePath = "BugReport" + (DateUtil.format(new Date(),"yyyyMMddHmmss"))+".txt";
...
/**
 * 有Hutool工具包中，使用FileUtil类进行文件操作时，通过不需要显示地”关闭“文件
 * 这是因为Hutool在内部处理文件I/O时，已经考虑了资源的自动管理和释放
 *
 * 具体来说，当你使用FileUtil的静态方法（如writeString,appendString,readUtf8String等）时，
 * 这些方法会在执行完毕后自动关闭与文件相磁的流和资源。因此，你不需要（也不能）
 * 调用类型close这样的方法来关闭文件
 *
 * 这里因为这些静态方法通常使用Java的try-with-resources语句或者其他类似的机制来确保资源在
 * 不再需要时得到释放，try-with-resource是Java 7及更版本中引入的一个特性
